...
@Component({
  selector: 'app-guest-edit-profile-page',
  templateUrl: './guest-edit-profile-page.component.html',
  styleUrls: ['./guest-edit-profile-page.component.css']
})
export class GuestEditProfilePageComponent implements OnInit{
  ...
  update(){
    ...
    this.form_data.append('old_username', this.guest.username);
    ...
    this.form_data.append('active', ""+this.guest.active);

    this.users_service.update_user(this.form_data).subscribe(
      data => {
        if(data != null){
          // azuriraj recenzije, restoran narudzbine
          this.reservations_service.update_reservation(data, this.guest.username).subscribe(
            () => {
              
            }
          )
          this.restaurants_service.update_restaurant(data, this.guest.username).subscribe(
            () => {

            }
          )
          this.orders_service.update_order(data, this.guest.username).subscribe(
            () => {

            }
          )
          localStorage.setItem("logged", JSON.stringify(data));
          this.router.navigate(["guest", data.username]); // !!!
